...
function themeToJson(theme: Theme) {
  ...
  json.light.surfaceContainerLowest = hexFromArgb(
...
async function materialDynamicColors(
  from: string | File | Blob | Event | HTMLImageElement
  ...
  try {
    ...
    let blob = new Blob();
    if (typeof to === "string")
      blob = await fetch(to).then((response) => response.blob());
    if (to.size) blob = to;
    if (to.files && to.files[0]) blob = to.files[0];
    if (to.target && to.target.files && to.target.files[0])
      blob = to.target.files[0];
...
export const setTheme = async (
  ...
  ref.current?.setAttribute("style", _lastTheme[mode]);
};
